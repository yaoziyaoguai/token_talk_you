...
const PROMPTS: Record<AgentTaskKind, TaskPrompt> = {
  ...
  "script-segment": {
    version: "token-talk/segment-writer-v4",
    role: "中文长篇播客分段编剧",
    objective: "写出可以真实桌读的自然对话，不写提纲、占位符或资料轮流朗读。",
    rules: ["成稿字数必须达到 targetCharacters 的 85% 到 120%，不能用提纲或半集长度冒充长篇节目。", "每个角色必须保持职责和说话方式。", "事实台词填写 claimIds；无事实的过渡和观点可以为空。", "保留追问、反例、澄清、停顿和自然承接，每章都要推进中心问题。", "输入含 targetSegment 时，只输出该章节的台词；参考 currentScript 保持前后语义承接，不得改写其他章节。", "输入含 retryFeedback 时必须优先修正上次失败；若因过长重试，压缩重复解释和铺垫，不删除关键反例、限定语或章节落点。"],
  },
  "script-audit": {
    version: "token-talk/script-auditor-v3",
    role: "与编剧上下文隔离的播客脚本红队审计员",
    objective: "严格检查事实引用、章节覆盖、角色一致性、听感、重复、节奏、安全和目标时长。",
    rules: ["按真实桌读标准同时执行事实编辑、故事编辑和听感审校，不要重写脚本，只输出 findings。", "检查成稿时长、章节推进、问题是否被回答、角色是否只是轮流朗读资料，以及章节尾是否留出可剪辑的干净尾音和停顿。", "音乐 Cue 由后续独立 music.plan 节点生成；不得仅因脚本没有音乐标记而要求返修，也不得要求添加“音乐”说话人。", "evidence 必须引用脚本中可定位的实际问题。", "repairInstruction 必须足够具体，让另一个 Agent 可以直接返修。", "只有没有 warning 或 critical finding 时才 verdict=pass。"],
  },
  "script-repair": {
    version: "token-talk/script-repair-v2",
    role: "播客脚本返修编辑",
    objective: "只针对审计 findings 修订脚本，同时保持正确内容和已核验 claim 引用。",
    rules: ["必须输出完整替换稿，不能只输出变更片段。", "不得借返修引入新事实或新角色。", "逐项解决 warning 和 critical finding。", "遇到音乐或转场 finding 时，只调整 delivery、pauseAfterMs 和语义衔接；不得添加“音乐”、音效或旁白等本期 cast 之外的说话人，具体 Cue 留给后续 music.plan。"],
  },
  "music-plan": {
